...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_get_user_password(self):
        print "test_get_user_email"
        role = Role(name=Name("Admin"))
        user = User(name=Name("ambika"), email=Email("ambika@vlabs.ac.in"), password("ambika") role=role)
        user_password = user.get_password()
        self.assertEqual(user_password, user.check_password("ambika"))
#+END_SRC
...
#+BEGIN_SRC python :tangle tests/test_db.py :eval no
    def test_user_to_client(self):
        ...
        role = Role(name=Name("Admin"))
        user = User(name=Name("madhavi"), email=Email("madhavi@vlabs.ac.in"), password="madhavi", role = role)
        user.save()
        user = User.get_by_id(1)
...
def admin():
    if request.method == 'POST':
        ...
        name_data = json.dumps({"name": name, "user": {"id":session['id']}})
        r = requests.post(APP_URL+"/reference_documents",data=name_data)
        r_json = json.loads(r.text)
        ID = r_json['id']
        filename = secure_filename(file.filename)
        dir_path = "%s" % (UPLOAD_DIR_PATH)
        file_path = "%s%s" % (dir_path, filename)  
        file.save(os.getcwd()+"/build/code/src/"+file_path)
        path = {'path': file_path}
        print path
        response = requests.put(APP_URL + "/reference_documents/"+ str(ID), data=path)
...
def login():
    if request.method == 'POST':
        ...
        current_app.logger.debug("response from Persona = %s" %
                                 resp.json())
        if resp.ok:
            verification_data = resp.json()
        if verification_data['status'] == 'okay':
            url_for_getting_the_user = "%s/users?email=%s" % \
                                       (APP_URL, verification_data['email'])
            current_app.logger.debug("url_for_getting_the_user = %s" %
                                     url_for_getting_the_user)
            backend_resp = requests.get(url_for_getting_the_user)
            current_app.logger.debug("backend response = %s" %
                                     backend_resp.text)
